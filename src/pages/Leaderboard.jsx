      ...
      <table className="w-full text-left text-sm">
        ...
        <tbody>
          {leaderboard.slice(0, 50).map((user, idx) => (
            <tr key={idx} className={user.username === getUserData()?.username ? 'text-pink-400 font-bold' : ''}>
              <td>{idx + 1}</td>
              <td>{user.username}</td>
              <td>{user.score}</td>
            </tr>
          ...
          {currentUserRank > 50 && (
            <>
              <tr><td colSpan="3" className="text-center text-gray-500">...</td></tr>
              <tr className="text-pink-400 font-bold">
                <td>{currentUserRank}</td>
                <td>{getUserData()?.username}</td>
                <td>{getUserData()?.games?.QuickMath?.bestScore || 0}</td>
              </tr>
            </>
          )}
        </tbody>
